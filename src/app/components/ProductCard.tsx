...
const Badges = ({ product, discountPct }: { product: Product; discountPct: number | null }) => (
...
const MemoizedBadges = ({ product, discountPct }: { product: Product; discountPct: number | null }) => {
  return <Badges product={product} discountPct={discountPct} />;
};

MemoizedBadges.displayName = 'Badges';

const WishlistBtn = ({ product, wishlisted, toggleWishlist }: { product: Product; wishlisted: boolean; toggleWishlist: (p: Product) => void }) => (
  <button
...
const MemoizedWishlistBtn = ({ product, wishlisted, toggleWishlist }: { product: Product; wishlisted: boolean; toggleWishlist: (p: Product) => void }) => {
  return <WishlistBtn product={product} wishlisted={wishlisted} toggleWishlist={toggleWishlist} />;
};

MemoizedWishlistBtn.displayName = 'WishlistBtn';

const QuickAdd = ({ product, showSizes, setShowSizes, addedSize, handleQuickAdd }: {
  product: Product;
  showSizes: boolean;
...
const MemoizedInfoSection = ({ product }: { product: Product }) => {
  return <InfoSection product={product} />;
};

MemoizedInfoSection.displayName = 'InfoSection';

// ── Main component ───────────────────────────────────────────────────────────
function ProductCardComponent({ product, dark }: ProductCardProps) {
  ...
  const hasMultipleImages = useMemo(() => !!(product.images && product.images.length >= 2), [product.images]);
    ...
    >
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="relative overflow-hidden" style={{ aspectRatio: '4/5', background: '#f5f5f5' }}>
          ...
          <MemoizedBadges product={product} discountPct={discountPct} />
          <MemoizedWishlistBtn product={product} wishlisted={wishlisted} toggleWishlist={toggleWishlist} />
          <QuickAdd product={product} showSizes={showSizes} setShowSizes={setShowSizes} addedSize={addedSize} handleQuickAdd={handleQuickAdd} />
        </div>
        <MemoizedInfoSection product={product} />
      </Link>
    </div>
  );
}
